    ...
    def create() {
        script.writeFile([
            ...
            "text" : script.generateConfig(
                "dcos-cli-${label}-${script.env.BRANCH_NAME}-${script.env.BUILD_ID}-${createAttempts}",
                "${templateUrl}")])
        launch_create()
        createAttempts++
    }
...
def testBuilder(String platform, String version, String templateUrl, String nodeId, String workspace = null) {
    return { Closure _body ->
        ...
        return {
            ...
            def cluster = new TestCluster(this, "${platform}-${version}", templateUrl)
...
for (version in DCOS_VERSIONS) {
    testBuilders[version] = [:]
    testBuilders["linux-tests-${version}"]  = linuxTestBuilder(version, TEMPLATE_URLS[version])
    testBuilders["mac-tests-${version}"]  = macTestBuilder(version, TEMPLATE_URLS[version])
    testBuilders["windows-tests-${version}"]  = windowsTestBuilder(version, TEMPLATE_URLS[version])
}
